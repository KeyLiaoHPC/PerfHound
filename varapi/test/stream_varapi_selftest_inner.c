...
main() {
    ...
    while (cur_bytes < 10485760) {
            ...
            for (k=0; k<NTIMES; k++) {
                for (j=0; j<4; j++) {
                    ...
                    for (i=0; i<numranks; i++) {
                        ...
                        tmin = MAX(tmin, TimesByRank[4*NTIMES*i+j*NTIMES+k]);
                    }
        ...
        sprintf(myfile, "./data_inner/%d_%llubytes_0.csv", myrank, cur_bytes);
        fp = fopen(myfile, "w");
        for (j = 0; j < NTIMES; j ++) {
        ...
        sprintf(myfile, "./data_inner/%d_%llubytes_1.csv", myrank, cur_bytes);
        fp = fopen(myfile, "w");
        for (j = 0; j < NTIMES; j ++) {
            fprintf(fp, "%llu,%llu,%llu,%llu\n", times[0][j], times[1][j], times[2][j], times[3][j]);
        }
