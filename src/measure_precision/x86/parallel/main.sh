...
function compile_and_run() {
    ...
    output_dir="./data/${tool}/arrlen_${arrlen}/${mode}_${kernel}_test_6248/round_${round}/run_${run_id}"
    if [ ! -d ${output_dir} ]; then
        exe=./${tool}_${mode}_${kernel}_${round}_mpi.x
        flags="-O3 -std=gnu99 -DTOOL=$tool -DMODE=$mode -DKNAME=$kernel -DROUND=$round -DARRLEN=$arrlen -DNMEASURE=$measure_num"
        echo "tool=${tool}, mode=${mode}, kernel=${kernel}, round=${round}, arrlen=${arrlen}"
        mpicc ${flags} -o $exe $src_file -lperfhound_mpi
        mpirun -np ${total_cores} --mca btl ^openib --bind-to core --map-by slot:pe=1 ${exe}
        rm ${exe}
    fi
}
